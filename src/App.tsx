...
const vfdSymptomLabels: Record<VfdSymptom, string> = {
  'Drive trip ediyor': 'Sürücü hata verip duruyor',
  'Motor çalışmıyor': 'Motor çalışmıyor',
  'Motor düşük hızda çalışıyor': 'Motor düşük hızda çalışıyor',
  'Motor aşırı akım çekiyor': 'Motor aşırı akım çekiyor',
  'Drive aşırı ısınıyor': 'Sürücü aşırı ısınıyor',
  'Haberleşme hatası var': 'Haberleşme hatası var',
  'Hız referansı gelmiyor': 'Hız referansı gelmiyor',
  'Drive hazır değil': 'Sürücü hazır değil',
}
...
function KnowledgeBase({ onOpenDiagnosis, initialQuery = '' }: { onOpenDiagnosis: (entry: ManufacturerFaultCode) => void; initialQuery?: string }) {
  ...
  const entries = manufacturerFaultCodes
    ...
    .map(({ entry }) => entry)
  const hasActiveFilters = query.trim() !== '' || manufacturer !== 'all' || modelFamily !== 'all' || category !== 'all'
  const clearFilters = () => {
    setQuery('')
    setManufacturer('all')
    setModelFamily('all')
  ...
  return <section className="knowledge-base" aria-labelledby="knowledge-title">
    ...
    <div className="knowledge-results-summary"><span>{entries.length} kayıt</span>{hasActiveFilters && <button type="button" onClick={clearFilters}>Filtreleri Temizle</button>}</div>
    <div className="knowledge-results">
      {entries.map((entry) => <article className="knowledge-card" key={entry.manufacturer + entry.modelFamily + entry.code}>
        <div className="knowledge-card-top"><div><span className="verified-source">Doğrulanmış üretici kaynağı</span><h2>{entry.code} — {entry.titleTr}</h2><p>{entry.manufacturer} <b>·</b> {entry.modelFamily}</p><span className="fault-category-badge">{entry.category}</span></div><div className="knowledge-actions"><button type="button" className="open-diagnosis-button" onClick={() => onOpenDiagnosis(entry)}>Teşhiste Kullan</button><button type="button" className="detail-button" onClick={() => setExpandedCode(expandedCode === entry.code ? null : entry.code)}>Ayrıntıları Gör</button></div></div>
        <p className="knowledge-description">{entry.descriptionTr}</p>
        {expandedCode === entry.code && <div className="knowledge-detail">{entry.troubleshooting ? <>{(entry.safetyNoteTr || entry.troubleshooting.safetyNoteTr) && <p className="entry-safety-note">{entry.safetyNoteTr || entry.troubleshooting.safetyNoteTr}</p>}<div className="troubleshooting-section"><strong>Olası Nedenler</strong><ul>{entry.troubleshooting.probableCausesTr.map((cause) => <li key={cause}>{cause}</li>)}</ul></div><div className="troubleshooting-section"><strong>Kontrol Sırası</strong><div className="troubleshooting-steps">{entry.troubleshooting.steps.map((step) => <article key={step.order} className={step.safetyCritical ? 'safety-critical-step' : ''}><span className="troubleshooting-step-number">{step.order}</span><div><b>{step.titleTr}</b><p>{step.detailTr}</p>{step.parameterCodes && <div className="parameter-chips">{step.parameterCodes.map((code) => <span key={code}>{code}</span>)}</div>}</div></article>)}</div></div>{entry.relatedParameters && <div className="related-parameters"><strong>İlgili Parametreler</strong><ul>{entry.relatedParameters.map((parameter) => <li key={parameter.code}><span><b>{parameter.code}</b> — {parameter.nameTr}</span><small>{parameter.purposeTr}</small></li>)}</ul></div>}{entry.troubleshooting.escalationTr?.length && <div className="troubleshooting-section escalation-section"><strong>Ne Zaman İleri İnceleme / Servis Gerekir</strong><ul>{entry.troubleshooting.escalationTr.map((item) => <li key={item}>{item}</li>)}</ul></div>}</> : <><div className="checks"><strong>Önerilen kontroller</strong><ul>{entry.recommendedChecks.map((check) => <li key={check}>{check}</li>)}</ul></div>{entry.relatedParameters && <div className="related-parameters"><strong>İlgili Parametreler</strong><ul>{entry.relatedParameters.map((parameter) => <li key={parameter.code}><span><b>{parameter.code}</b> — {parameter.nameTr}</span><small>{parameter.purposeTr}</small></li>)}</ul></div>}{entry.safetyNoteTr && <p className="entry-safety-note">{entry.safetyNoteTr}</p>}</>}{entry.sourceScope && <p className="source-scope">Kaynak kapsamı: {entry.sourceScope}</p>}<a href={entry.sourceUrl} target="_blank" rel="noopener noreferrer">{entry.manufacturer === 'Siemens' ? 'Resmî Siemens kaynağını aç' : 'Resmî üretici kaynağını aç'}</a></div>}
      </article>)}
...
function App() {
  const [activeView, setActiveView] = useState<'dashboard' | 'diagnosis' | 'knowledge' | 'commissioning' | 'history' | 'settings'>('dashboard')
  ...
  const [isVfdAdvancedOpen, setVfdAdvancedOpen] = useState(false)
  const [isMotorAdvancedOpen, setMotorAdvancedOpen] = useState(false)
  ...
  const resetDiagnosisFormForEquipment = (nextEquipment: string) => {
    setEquipment(nextEquipment)
    setBrandModel('')
    setMotorSelectedSymptoms([]); setVfdSelectedSymptoms([])
    setNominalCurrent(''); setMeasuredCurrent(''); setL1Current(''); setL2Current(''); setL3Current(''); setL1L2Voltage(''); setL2L3Voltage(''); setL3L1Voltage(''); setMotorTemperature(''); setVibration('')
    setFaultCode(''); setVfdManufacturer(''); setVfdModelFamily(''); setVfdVoltageClass('unknown'); setDcBusVoltage(''); setOutputCurrent(''); setMotorNominalCurrent(''); setOutputFrequency(''); setDriveTemperature('')
    setVfdAdvancedOpen(false); setMotorAdvancedOpen(false)
    setResults(null); setManufacturerFault(null); setUnknownFaultCode(false); setValidationMessage(''); setMeasurementWarning(''); setMeasurementInfo([]); setHistorySaveMessage(''); setLastSavedSignature(null); setDiagnosisSourceContext('manual-diagnosis')
  }
  ...
  const openKnowledgeEntryInDiagnosis = (entry: ManufacturerFaultCode) => {
    ...
    setVfdAdvancedOpen(false)
    setMotorAdvancedOpen(false)
  ...
  const reopenHistoryRecord = (record: DiagnosisHistoryRecord) => {
    ...
    setFaultCode(record.manufacturerFaultResult?.code ?? record.faultCode ?? '')
    ...
    if (record.equipmentType === VFD_EQUIPMENT) {
      setVfdManufacturer((record.manufacturerFaultResult?.manufacturer.toLocaleLowerCase('tr-TR') as VfdManufacturer | undefined) ?? (record.manufacturer as VfdManufacturer | undefined) ?? '')
      setVfdModelFamily((record.manufacturerFaultResult?.modelFamily as VfdModelFamily | undefined) ?? (record.modelFamily as VfdModelFamily | undefined) ?? '')
      ...
      setVfdAdvancedOpen(record.symptoms.length > 0 || measure('voltageClass') !== '' || ['dcBusVoltage', 'outputCurrent', 'motorNominalCurrent', 'outputFrequency', 'driveTemperature'].some((key) => measure(key) !== ''))
      setMotorAdvancedOpen(false)
    } else {
      ...
      setMotorAdvancedOpen(['l1Current', 'l2Current', 'l3Current', 'l1l2Voltage', 'l2l3Voltage', 'l3l1Voltage', 'motorTemperature', 'vibration'].some((key) => measure(key) !== ''))
      setVfdAdvancedOpen(false)
    }
  ...
  useEffect(() => {
    ...
    // The pending record is the explicit reopen trigger; form state is read when the timer runs.
      ...
      <div className="workspace">
        <section className="form-card" aria-labelledby="form-title">
          ...
          <form onSubmit={(event) => { event.preventDefault(); analyze() }}>
            <div className="form-grid"><label>EKİPMAN TÜRÜ<select value={equipment} onChange={(event) => resetDiagnosisFormForEquipment(event.target.value)}><option value="" disabled>Ekipman seçiniz</option>{equipmentOptions.map((option) => <option key={option}>{option}</option>)}</select></label>{isVfd ? <label>ÜRETİCİ<select value={vfdManufacturer} onChange={(event) => { const manufacturer = event.target.value as VfdManufacturer | ''; setVfdManufacturer(manufacturer); setVfdModelFamily(manufacturer === 'siemens' ? 'SINAMICS G120' : manufacturer === 'yaskawa' ? 'V1000' : manufacturer === 'danfoss' ? 'VLT AutomationDrive FC 302' : '') }}><option value="">Seçiniz</option><option value="siemens">Siemens</option><option value="yaskawa">Yaskawa</option><option value="danfoss">Danfoss</option></select></label> : <label>MARKA / MODEL<input value={brandModel} onChange={(event) => setBrandModel(event.target.value)} placeholder="Örn: Siemens 1LE1001" /></label>}</div>
            {!isVfd && <><div className="form-grid electrical-fields"><label>NOMİNAL AKIM <div className="input-suffix"><input type="number" value={nominalCurrent} onChange={(event) => setNominalCurrent(event.target.value)} placeholder="Örn: 10" min="0" step="0.01" /><span>A</span></div></label><label>ÖLÇÜLEN AKIM <div className="input-suffix"><input type="number" value={measuredCurrent} onChange={(event) => setMeasuredCurrent(event.target.value)} placeholder="Örn: 12" min="0" step="0.01" /><span>A</span></div></label></div><fieldset><legend>BELİRTİLER <span>Uygun olanları seçin</span></legend><div className="symptoms">{motorSymptoms.map((symptom) => <label className="checkbox-label" key={symptom}><input type="checkbox" checked={motorSelectedSymptoms.includes(symptom)} onChange={() => toggleMotorSymptom(symptom)} /><span className="checkmark">✓</span>{symptom}</label>)}</div></fieldset><label className="full-field">PROBLEM TANIMI<textarea rows={3} placeholder="Gözlemlediğiniz problemi kısaca açıklayın..." /></label><button className="disclosure-button" type="button" onClick={() => setMotorAdvancedOpen((open) => !open)} aria-expanded={isMotorAdvancedOpen}>{isMotorAdvancedOpen ? '− Gelişmiş ölçümleri gizle' : '+ Gelişmiş ölçümler'}</button>{isMotorAdvancedOpen && <div className="advanced-fields"><fieldset className="measurement-section"><legend>FAZ ÖLÇÜMLERİ <span>İsteğe bağlı</span></legend><div className="measurement-grid"><label>L1 AKIM <div className="input-suffix"><input type="number" value={l1Current} onChange={(event) => setL1Current(event.target.value)} placeholder="Örn: 8.5" min="0" step="0.01" /><span>A</span></div></label><label>L2 AKIM <div className="input-suffix"><input type="number" value={l2Current} onChange={(event) => setL2Current(event.target.value)} placeholder="Örn: 8.5" min="0" step="0.01" /><span>A</span></div></label><label>L3 AKIM <div className="input-suffix"><input type="number" value={l3Current} onChange={(event) => setL3Current(event.target.value)} placeholder="Örn: 8.5" min="0" step="0.01" /><span>A</span></div></label></div><div className="measurement-grid"><label>L1-L2 GERİLİM <div className="input-suffix"><input type="number" value={l1L2Voltage} onChange={(event) => setL1L2Voltage(event.target.value)} placeholder="Örn: 400" min="0" step="0.1" /><span>V</span></div></label><label>L2-L3 GERİLİM <div className="input-suffix"><input type="number" value={l2L3Voltage} onChange={(event) => setL2L3Voltage(event.target.value)} placeholder="Örn: 400" min="0" step="0.1" /><span>V</span></div></label><label>L3-L1 GERİLİM <div className="input-suffix"><input type="number" value={l3L1Voltage} onChange={(event) => setL3L1Voltage(event.target.value)} placeholder="Örn: 400" min="0" step="0.1" /><span>V</span></div></label></div></fieldset><div className="form-grid auxiliary-measurements"><label>MOTOR SICAKLIĞI <div className="input-suffix"><input type="number" value={motorTemperature} onChange={(event) => setMotorTemperature(event.target.value)} placeholder="Örn: 65" min="0" step="0.1" /><span>°C</span></div></label><label>TİTREŞİM <div className="input-suffix"><input type="number" value={vibration} onChange={(event) => setVibration(event.target.value)} placeholder="Örn: 3.2" min="0" step="0.1" /><span>mm/s</span></div></label></div></div>}</>}
            {isVfd && <><div className="form-grid vfd-model-field"><label>MODEL AİLESİ <select value={vfdModelFamily} onChange={(event) => setVfdModelFamily(event.target.value as VfdModelFamily | '')} disabled={!vfdManufacturer}><option value="">Seçiniz</option>{vfdManufacturer === 'siemens' && <option value="SINAMICS G120">SINAMICS G120</option>}{vfdManufacturer === 'yaskawa' && <option value="V1000">V1000</option>}{vfdManufacturer === 'danfoss' && <option value="VLT AutomationDrive FC 302">VLT AutomationDrive FC 302</option>}</select></label><label>ARIZA KODU <input value={faultCode} onChange={(event) => setFaultCode(event.target.value)} placeholder="İsteğe bağlı; örn: F0001" /></label></div><button className="disclosure-button" type="button" onClick={() => setVfdAdvancedOpen((open) => !open)} aria-expanded={isVfdAdvancedOpen}>{isVfdAdvancedOpen ? '− Belirti ve ölçümleri gizle' : '+ Belirti ve ölçüm ekle'}</button>{isVfdAdvancedOpen && <div className="advanced-fields"><fieldset><legend>VFD BELİRTİLERİ <span>Uygun olanları seçin</span></legend><div className="symptoms">{vfdSymptoms.map((symptom) => <label className="checkbox-label" key={symptom}><input type="checkbox" checked={vfdSelectedSymptoms.includes(symptom)} onChange={() => toggleVfdSymptom(symptom)} /><span className="checkmark">✓</span>{vfdSymptomLabels[symptom]}</label>)}</div></fieldset><fieldset className="measurement-section"><legend>VFD ÖLÇÜMLERİ <span>İsteğe bağlı</span></legend><div className="form-grid"><label>GERİLİM SINIFI <select value={vfdVoltageClass} onChange={(event) => setVfdVoltageClass(event.target.value as VfdVoltageClass)}><option value="unknown">Bilinmiyor</option><option value="230">230 V Class</option><option value="400">400 V Class</option><option value="480">480 V Class</option></select></label><label>DC BARA GERİLİMİ <div className="input-suffix"><input type="number" value={dcBusVoltage} onChange={(event) => setDcBusVoltage(event.target.value)} placeholder="Örn: 600" min="0" step="0.1" /><span>V</span></div></label><label>ÇIKIŞ AKIMI <div className="input-suffix"><input type="number" value={outputCurrent} onChange={(event) => setOutputCurrent(event.target.value)} placeholder="Örn: 12" min="0" step="0.01" /><span>A</span></div></label><label>MOTOR NOMİNAL AKIMI <div className="input-suffix"><input type="number" value={motorNominalCurrent} onChange={(event) => setMotorNominalCurrent(event.target.value)} placeholder="Örn: 10" min="0" step="0.01" /><span>A</span></div></label><label>ÇIKIŞ FREKANSI <div className="input-suffix"><input type="number" value={outputFrequency} onChange={(event) => setOutputFrequency(event.target.value)} placeholder="Örn: 50" min="0" step="0.1" /><span>Hz</span></div></label><label>SÜRÜCÜ SICAKLIĞI <div className="input-suffix"><input type="number" value={driveTemperature} onChange={(event) => setDriveTemperature(event.target.value)} placeholder="Örn: 65" min="0" step="0.1" /><span>°C</span></div></label></div></fieldset></div>}</>}
            <button className="analyze-button" type="submit"><span>⌁</span> {isVfd && faultCode.trim() ? 'HATA KODUNU İNCELE' : 'ARIZAYI ANALİZ ET'} <b>→</b></button>
          </form>
        </section>
